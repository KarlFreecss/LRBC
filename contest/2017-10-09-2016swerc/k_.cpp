...
int dfs(int ori, int v, const int S){
  vis[v] = true;
  area[v] = S;
...
int hasLoop2(){
  area.resize(p2d.size(), 0);
  vis.resize(p2d.size(), false);
...
int work(){
  ...
  for (int i = 0; i < K; ++i) {
    ...
    if (dic3D.count(p3d1) == 0) {
      dic3D[p3d1] = dic3D.size() + 1;
    }
    if (dic3D.count(p3d2) == 0) {
      dic3D[p3d2] = dic3D.size() + 1;
    }
    if (dic2D.count(p2d1) == 0) {
      dic2D[p2d1] = dic2D.size();
      p2d.push_back(p2d1);
    }
    if (dic2D.count(p2d2) == 0) {
      dic2D[p2d2] = dic2D.size();
      p2d.push_back(p2d2);
    }
    a[i] = dic3D[p3d1];
    b[i] = dic3D[p3d2];
